...
import { BaseKernelManager, Context } from "@inline-repl/core";
import { KernelError } from "@inline-repl/core";

export class RubyKernelManager extends BaseKernelManager {
  ...
  async selectKernel(): Promise<Kernel | undefined> {
    ...
    if (workspace.notebookDocuments.length === 0) {
      throw new KernelError(
        "No notebooks open. Open a notebook with Ruby kernel first."
      );
    }
